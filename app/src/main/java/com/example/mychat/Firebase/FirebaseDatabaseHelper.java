...
import com.example.mychat.Adapters.RecyclerViewAdapter;
import com.example.mychat.ProfileFragment;
...
public class FirebaseDatabaseHelper {
    ...
    public void isUserKeyExist(final String uid, final Context context, final RecyclerView recyclerView){
        databaseReference.child("users").addChildEventListener(new ChildEventListener() {
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                ...
                List<ProfileFragment> userData = adapterSourceData(dataSnapshot, uid);
    ...
    private List<ProfileFragment> adapterSourceData(DataSnapshot dataSnapshot, String uId){
        List<ProfileFragment> allUserData = new ArrayList<ProfileFragment>();
        if(dataSnapshot.getKey().equals(uId)){
            FirebaseUserEntity userInformation = dataSnapshot.getValue(FirebaseUserEntity.class);
            //allUserData.add(new ProfileFragment(Helper.NAME, userInformation.getName()));
            //allUserData.add(new ProfileFragment(Helper.EMAIL, userInformation.getEmail()));
            //allUserData.add(new ProfileFragment(Helper.BIRTHDAY, userInformation.getBirthday()));
            //allUserData.add(new ProfileFragment(Helper.PHONE_NUMBER, userInformation.getPhone()));
        }
        return allUserData;
    }
}